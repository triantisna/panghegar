// src/app/dashboard/tech/page.js
"use client";
    ...
    const fetchUserAndProjects = async () => {
      try {
        const [sessionRes, projRes] = await Promise.all([
          ...
          fetch("/api/v1/projects/for-tech"),
        ]);
      ...
      <main className="mx-auto max-w-6xl px-4 pb-16 pt-4">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-base md:text-lg font-semibold text-slate-900">
            Proyek yang Anda Ditugaskan
          </h2>
        </div>
        <p className="mt-1 text-xs text-slate-500">
          Daftar proyek tempat Anda ter-assign sebagai teknisi.
        </p>
        ...
        {projects.length === 0 ? (
          ...
          <ProjectListGrid projects={projects} variant="TECH" />
        )}
      </main>
    </div>
  );
